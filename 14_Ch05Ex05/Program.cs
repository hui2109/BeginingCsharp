            ...
            foreach (string friendName in friendNames)
            {
                switch (friendName)
                {
                    case string t when t.StartsWith("T"):
                        Console.WriteLine("This friend's name starts with a 'T': " + $"{friendName} and is {t.Length - 1} letters long.");
                        break;
                    case string e when e.Length == 0:
                        Console.WriteLine("There is a string in the array with no value.");
                        break;
                    case null:
                        Console.WriteLine("There is a 'null' value in the array.");
                        break;
                    case var x:
                        Console.WriteLine("There is the var pattern of type: " + $"{x.GetType().Name}");
                        break;
                    default:
                        break;
                }
